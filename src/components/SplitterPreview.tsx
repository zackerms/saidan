...
export function SplitterPreview({
  ...
  const columnPositions = useMemo(() => {
    return columnWidths.map((width, index) => {
      const offsetX = columnWidths.slice(0, index).reduce((acc, curr) => acc + curr, 0);
  ...
  const rowPositionToCut = useMemo(() => {
    if(numberOfRowsToCut === null || numberOfRowsToCut === 0 || numberOfRowsToCut > rows.length) return null;
  ...
  const handleOnColumnCutLineClick = useCallback((index: number) => {
      ...
      <div className="relative" ref={tableRef}>
        ...
        {Array.from({ length: Math.max(0, columnCount - 1) }, (_, index) => index).map((lineIndex) => {
          return <VerticalCut
            ...
            isSelected={numberOfColumnsToCut !== null && numberOfColumnsToCut - 1 <= lineIndex}
...
const Row = ({ 
  ...
  const actualRowIndex = useMemo(() => displayStartRowIndex + rowIndex, [displayStartRowIndex, rowIndex]);
  const isInCutRowRange = useMemo(() => numberOfRowsToCut > 0 && actualRowIndex < numberOfRowsToCut, [numberOfRowsToCut, actualRowIndex]);
...
const Cell = ({
  ...
  const isCutOnBottomBorder = useMemo(() => numberOfRowsToCut !== null && numberOfRowsToCut - 1 === actualRowIndex, [numberOfRowsToCut, actualRowIndex]);
  const isCutOnRightBorder = useMemo(() => numberOfColumnsToCut !== null && numberOfColumnsToCut - 1 <= columnIndex, [numberOfColumnsToCut, columnIndex]);
  const isInCutColumnRange = useMemo(() => numberOfColumnsToCut !== null && numberOfColumnsToCut <= columnIndex, [numberOfColumnsToCut, columnIndex]);
  ...
  return (
    <td 
      className="relative"
      style={{ height: rowHeight, boxSizing: 'border-box', paddingLeft: 10, paddingRight: 10, paddingTop: 0, paddingBottom: 0 }}
      ref={cellRef}
    >
           {isCutOnBottomBorder && (
                    <div
